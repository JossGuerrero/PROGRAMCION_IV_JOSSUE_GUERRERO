...
class _ServicioEstadoState extends State<ServicioEstado> {
  bool _activo    = true;
  int  _llamadosTaller = 0;

  // Cambiado de 3 a 1
  static const int _maxLlamadosTaller = 1;

  String nivel = 'normal';

  void _actualizarNivel() {
    if (_llamadosTaller >= 2) {
      nivel = 'critico';
    } else if (_llamadosTaller >= 1) {
      nivel = 'warning';
    } else {
      nivel = 'normal';
    }
  ...
  Widget build(BuildContext context) {
    final enLimite = _llamadosTaller >= _maxLlamadosTaller;
