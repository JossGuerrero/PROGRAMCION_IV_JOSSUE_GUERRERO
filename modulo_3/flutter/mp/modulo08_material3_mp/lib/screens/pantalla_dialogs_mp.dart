...
class PantallaDialogs extends StatelessWidget {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(esError
            ? 'Error: no se pudo conectar con la aerolínea'
            : 'Vuelo actualizado'),
  ...
  void _mostrarAlertaInformativa(BuildContext context) {
    showDialog<void>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: const Text('Actualización de Vuelo'),
        content: const Text(
          'Se han actualizado los horarios de tu vuelo. '
          'Por favor, revisa tu itinerario antes de llegar al aeropuerto.',
        ),
        actions: [
          TextButton(
   ...
   void _mostrarAlertaBotonDos(BuildContext context) {
    showDialog<void>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: const Text('Cambio de puerta de embarque'),
        content: const Text(
          'La puerta de embarque de tu vuelo ha cambiado. '
          'Por favor dirígete a la nueva puerta cuanto antes.',
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(ctx),
  ...
  // Alerta de Confirmación (Cancelar vuelo)
  Future<void> _mostrarConfirmacion(BuildContext context) async {
    final confirmar = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
          '¿Estás seguro de que deseas cancelar el vuelo AV204?\n'
  ...
  Future<void> _mostrarFormulario(BuildContext context) async {
        ...
        content: Form(
          ...
          child: Column(
            ...
            children: [
              ...
              TextFormField(
                controller: ctrlIp,
                decoration: const InputDecoration(labelText: 'Número de asiento'),
                validator: (v) {
                  if (v == null || v.isEmpty) return 'Campo requerido';
                  if (!RegExp(r'^[0-9]{1,2}[A-F]$').hasMatch(v)) return 'Formato: 12A';
                  return null;
                },
              ),
            ],
    ...
    if (ctrlNombre.text.isNotEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Pasajero "${ctrlNombre.text}" agregado')),
      );
    }
  }
      ...
      body: ListView(
        ...
        children: [
          ...
          FilledButton.icon(
            onPressed: () => _mostrarSnackBar(context),
            icon: const Icon(Icons.check_circle_outline),
            label: const Text('Notificar cambio de horario'),
          ),
          const SizedBox(height: 8),
          FilledButton.icon(
            style: FilledButton.styleFrom(
          ...
          FilledButton.icon(
            ...
            label: const Text('Cambio de puerta'),
          ),
          const SizedBox(height: 8),
          ...
          OutlinedButton.icon(
            ...
            label: const Text('Cancelar vuelo (Confirmación)'),
          ),
          const SizedBox(height: 8),
          ...
          FilledButton.tonal(
            ...
            child: const Text('Agregar pasajero (Formulario)'),
          ),
        ],
      ),
    );
