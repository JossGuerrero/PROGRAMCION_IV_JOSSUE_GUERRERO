...
class PantallaMetricas extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Estado de la flota de vuelos'),
        actions: [
          IconButton(
...
class _TarjetaMetrica extends StatelessWidget {
  final MetricaBus metrica;
  const _TarjetaMetrica({required this.metrica});

  @override
  Widget build(BuildContext context) {
    final cs         = Theme.of(context).colorScheme;
    final cpuCritica = metrica.velocidad > 85;
    final ramCritica = metrica.pasajeros > 90;
    final ssdCritica = metrica.combustible > 90; 
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                Icon(Icons.flight, color: esCritico ? cs.error : cs.primary, size: 18),
            ...
            Text(
              'Combustible: ${metrica.combustible.toStringAsFixed(0)}% usado',
              style: TextStyle(fontSize: 12, color: cs.onSurfaceVariant),
            ),
            const SizedBox(height: 10),
            _Barra('Turbulencia', metrica.velocidad, cpuCritica),
            const SizedBox(height: 4),
            _Barra('Ocupación', metrica.pasajeros, ramCritica),
            const SizedBox(height: 4),
            _Barra('Combustible', metrica.combustible, ssdCritica),
          ],
        ),
      ),
    );
