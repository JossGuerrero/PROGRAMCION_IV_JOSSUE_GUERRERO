...
class PantallaPaso2Users extends StatelessWidget {
  ...
  Future<List<PilotoDto>> _fetchConductores() async {
    final res = await http.get(
      Uri.parse('https://jsonplaceholder.typicode.com/users?_limit=15'),
    );
    final lista = jsonDecode(res.body) as List<dynamic>;
    return lista
        .map((e) => PilotoDto.fromJson(e as Map<String, dynamic>))
        .toList();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Paso 2 · DTO pilotos'),
        leading: BackButton(onPressed: () => context.go('/')),
      ),
      body: FutureBuilder<List<PilotoDto>>(
            ...
            children: [
              Padding(
                ...
                child: Row(
                  children: [
                    Chip(
                      label: Text('${conductores.length} pilotos'),
